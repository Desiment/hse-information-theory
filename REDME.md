...
1. Подтянуть все субмодули:
   ```bash
   git submodule update --init --recursive
   ```

2. Установить необходимые зависимости для пакетов:
   - `minted` (может потребовать `pygmentize`)
   - `memoize` (потребует Perl-библиотеку `PDF::Builder`)

3. Убедиться, что установлена свежая версия `latexmk` (4.85+).

4. Использовать движок LuaHBTeX версии 1.18.0 или выше.

5. Для сборки использовать команду `latexmk` из корневой директории (где расположен файл `latexmkrc`).

6. Рекомендуется использовать редактор с поддержкой магических команд (например, VS Code с расширением LaTeX Workshop).
